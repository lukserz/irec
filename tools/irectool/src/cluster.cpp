...
{
    IRECLIB_CASSERT(feats.size() != 0, "The dataset can't be empty");
    for (unsigned long i = 0; i < feats.size(); ++i)
    {
        IRECLIB_CASSERT(feats[i].size() == feats[0].size(), "All feature vectors must have the same length.");
    }
...
int cluster_dataset(
    const ireclib::command_line_parser& parser
)
{
    ...
    for (unsigned long c = 0; c < num_clusters; ++c)
    {
        ireclib::array<array2d<rgb_pixel> > temp;
