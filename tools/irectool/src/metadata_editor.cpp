    ...
    last_keyboard_jump_pos_update(0)
{
    ...
    if (!font_path.empty())
    {
        const auto custom_font = std::make_shared<ireclib::bdf_font>();
    ...
    ireclib::array<std::string>::expand_1a files;
...
{
    ireclib::queue<unsigned long>::kernel_1a list;
    ...
    ireclib::static_set<unsigned long>::kernel_1a to_remove;
    to_remove.load(list);
    std::vector<ireclib::image_dataset_metadata::image> images;
    for (unsigned long i = 0; i < metadata.images.size(); ++i)
    {
...
void propagate_boxes(
    ireclib::image_dataset_metadata::dataset& data,
    unsigned long prev,
    unsigned long next
)
{
    ...
    ireclib::load_image(img1, data.images[prev].filename);
    data.images[prev].width = img1.nc();
    data.images[prev].height = img1.nr();

    ireclib::load_image(img2, data.images[next].filename);
    ...
    for (unsigned long i = 0; i < data.images[prev].boxes.size(); ++i)
    {
        ...
        tracker.update(img2);
        ireclib::image_dataset_metadata::box box = data.images[prev].boxes[i];
...
bool has_label_or_all_boxes_labeled (
    const std::string& label,
    const ireclib::image_dataset_metadata::image& img 
)
{
    if (label.size() == 0)
        return true;
...
std::vector<ireclib::image_display::overlay_rect> get_overlays (
    const ireclib::image_dataset_metadata::image& data,
    color_mapper& string_to_color 
)
{
    std::vector<ireclib::image_display::overlay_rect> temp(data.boxes.size());
...
)
{
    ...
    try
    {
        ireclib::load_image(img, metadata.images[idx].filename);
...
{
    using namespace ireclib::image_dataset_metadata;
