...
)
{
    ...
    for (unsigned long i = 0; i < parser.number_of_arguments(); ++i)
    {
        ...
        catch (ireclib::file::file_not_found&)
        {
...
)
{
    ...
    ireclib::image_dataset_metadata::dataset data, data_with, data_without;
...
)
{
    ...
    ireclib::image_dataset_metadata::dataset data, data_train, data_test;
    load_image_dataset_metadata(data, parser[0]);
...
void print_all_labels (
    const ireclib::image_dataset_metadata::dataset& data
)
{
...
int resample_dataset(const command_line_parser& parser)
{
    ...
    ireclib::image_dataset_metadata::dataset data, resampled_data;
...
int tile_dataset(const command_line_parser& parser)
{
    ...
    load_image_dataset_metadata(data, parser[0]);
    locally_change_current_dir chdir(get_parent_directory(file(parser[0])));
    ireclib::array<array2d<rgb_pixel> > images;
    console_progress_indicator pbar(data.images.size());
    for (unsigned long i = 0; i < data.images.size(); ++i)
    {
        // don't even bother loading images that don't have objects.
        ...
        ireclib::array<array2d<rgb_pixel> > chips;
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (parser.option("v"))
        {
            ...
            cout << "Check for updates at http://ireclib.net\n\n";
            return EXIT_SUCCESS;
        }
        ...
        if (parser.option("rmempty"))
        {
            ...
            ireclib::image_dataset_metadata::dataset data, data2;
            load_image_dataset_metadata(data, parser[0]);
        ...
        if (parser.option("rmlabel"))
        {
            ...
            {
                std::vector<ireclib::image_dataset_metadata::box> boxes;
        ...
        if (parser.option("rmdupes"))
        {
            ...
            ireclib::image_dataset_metadata::dataset data, data_out;
