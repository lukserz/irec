...
// ----------------------------------------------------------------------------------------
    ...
    void parse_annotation_file(
        ...
        ireclib::image_dataset_metadata::image& img,
        std::string& dataset_name
    )
    {
        ifstream fin(file.c_str());
        if (!fin)
            throw ireclib::error("Unable to open file " + file);

        img = ireclib::image_dataset_metadata::image();
            ...
            if (words.size() > 2)
            {
                ...
                else if (words.size() > 4 && (words[2] == "for" || words[2] == "on") && words[3] == "object")
                {
                    ...
                    if (idx >= (long)img.boxes.size())
                        throw ireclib::error("Invalid object id number of " + words[4]);
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        while (true)
        {
            ...
            if (parent.is_root())
                throw ireclib::error("Can't figure out where the file " + image_name + " is located.");
...
)
{
    ...
    ireclib::image_dataset_metadata::dataset dataset;
