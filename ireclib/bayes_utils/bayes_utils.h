...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        )
        {
            ...
            IRECLIB_ASSERT( has_index(idx) == false ,
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( has_index(idx) == true ,
        ...
        { 
            // make sure requires clause is not broken
            IRECLIB_ASSERT(current_element_valid() == true,
    ...
    private:
        mutable ireclib::map<unsigned long, unsigned long>::kernel_1b_c vals;
    };
...
// ------------------------------------------------------------------------
    ...
    private:
        ...
        ireclib::map<assignment, double >::kernel_1b_c table;
    };
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( value < num_values() && 0.0 <= p && p <= 1.0 ,
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( value < num_values() && has_entry_for(value,ps) ,
    ...
    private:
        ireclib::map<assignment, matrix<double,1> >::kernel_1b_c table;
...
// ------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( new_value < table().num_values(),
...
// ------------------------------------------------------------------------
    ...
    namespace bayes_node_utils
    {
        ...
        { 
            // make sure requires clause is not broken
            IRECLIB_ASSERT( n < bn.number_of_nodes(),
    ...
    // ----------------------------------------------------------------------------------------
        ...
        { 
            // make sure requires clause is not broken
            IRECLIB_ASSERT( n < bn.number_of_nodes() && val < node_num_values(bn,n),
    ...
    // ----------------------------------------------------------------------------------------
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( n < bn.number_of_nodes() && value < node_num_values(bn,n),
            ...
            IRECLIB_ASSERT( parents.size() == bn.node(n).number_of_parents(),
                         "\tdouble bayes_node_utils::node_probability(bn, n, value, parents)"
                         << "\n\tInvalid arguments to this function"
                         << "\n\tn:                             " << n 
            ...
            while (parents.move_next())
            {
                ...
                IRECLIB_ASSERT( bn.has_edge(x, n),
                             "\tdouble bayes_node_utils::node_probability(bn, n, value, parents)"
                             << "\n\tInvalid arguments to this function"
                             << "\n\tn: " << n 
                             << "\n\tx: " << x 
                );
                IRECLIB_ASSERT( parents[x] < node_num_values(bn,x),
                             "\tdouble bayes_node_utils::node_probability(bn, n, value, parents)"
                             << "\n\tInvalid arguments to this function"
                             << "\n\tn:                     " << n 
                             << "\n\tx:                     " << x 
...
// ----------------------------------------------------------------------------------------
        ...
        { 
            ...
            IRECLIB_ASSERT( a.size() == bn.node(n).number_of_parents(),
                         "\tbool bayes_node_utils::node_next_parent_assignment(bn, n, a)"
                         << "\n\tInvalid arguments to this function"
                         << "\n\tn:                             " << n 
                         << "\n\ta.size():                      " << a.size()
            ...
            while (a.move_next())
            {
                ...
                IRECLIB_ASSERT( a[x] < node_num_values(bn,x),
                             "\tbool bayes_node_utils::node_next_parent_assignment(bn, n, a)"
                             << "\n\tInvalid arguments to this function"
                             << "\n\tn:                     " << n 
                             << "\n\tx:                     " << x 
...
// ----------------------------------------------------------------------------------------
    ...
    namespace bayesian_network_join_tree_helpers
    {
        ...
        // ----------------------------------------------------------------------------------------
            ...
            !*/
            {
                ...
                ireclib::map<unsigned long, unsigned long>::kernel_1b_c evidence;
                ...
                // we are done
                ireclib::array<ireclib::set<unsigned long>::compare_1b_c> remaining_msg_to_send;
                ireclib::array<ireclib::set<unsigned long>::compare_1b_c> remaining_msg_to_receive;
    ...
    public:
        ...
        )
        {
            ...
            for (unsigned long i = 0; i < bn.number_of_nodes(); ++i)
            {
                IRECLIB_ASSERT(bayes_node_utils::node_cpt_filled_out(bn,i) == true,
                        "\tbayesian_network_join_tree::bayesian_network_join_tree(bn,join_tree)"
                        << "\n\tYou have given an invalid bayesian network. "
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( idx < number_of_nodes() ,
