...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        requires
            - 0 < bits <= 32
            - v.size() > 1
            - vector_type == a std::vector or compatible type containing ireclib::matrix 
            ...
            - rand_type == a type that implements the ireclib/rand/rand_kernel_abstract.h interface
        ensures
        ...
        requires
            ...
            - for all valid i, j:
                ...
                - i.e. v contains only column vectors and all the column vectors
                  have the same non-zero length
        ensures
            - returns create_random_projection_hash(v,bits,ireclib::rand())
        ...
        requires
            ...
            - for all valid i, j:
                ...
                - i.e. v contains only column vectors and all the column vectors
                  have the same non-zero length
        ensures
            - returns create_max_margin_projection_hash(v,bits,C,ireclib::rand())
              (i.e. calls the above function with a default initialized random number generator)
    !*/
