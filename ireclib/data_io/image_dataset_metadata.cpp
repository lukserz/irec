...
namespace ireclib
{
    ...
    // ------------------------------------------------------------------------------------
        ...
        void create_image_metadata_stylesheet_file(const std::string& main_filename)
        {
            ...
            if (!fout)
                throw ireclib::error("ERROR: Unable to open image_metadata_stylesheet.xsl for writing.");

            fout << get_decoded_string();

            if (!fout)
                throw ireclib::error("ERROR: Unable to write to image_metadata_stylesheet.xsl.");
        }
        ...
        )
        {
            ...
            for (unsigned long i = 0; i < images.size(); ++i)
            {
                ...
                if (!fout)
                    throw ireclib::error("ERROR: Unable to write to " + filename + ".");
            }
            fout << "</images>\n";
            fout << "</dataset>";
        }
            ...
            )
            {
                try
                {
                    ...
                    if (name == "box")
                    {
                        ...
                        else throw ireclib::error("<box> missing required attribute 'top'");

                        if (atts.is_in_list("left")) temp_box.rect.left() = sa = atts["left"];
                        else throw ireclib::error("<box> missing required attribute 'left'");

                        if (atts.is_in_list("width")) temp_box.rect.right() = sa = atts["width"];
                        else throw ireclib::error("<box> missing required attribute 'width'");

                        if (atts.is_in_list("height")) temp_box.rect.bottom() = sa = atts["height"];
                        else throw ireclib::error("<box> missing required attribute 'height'");
                        ...
                        if (atts.is_in_list("gender"))  
                        {
                            if (atts["gender"] == "male")
                                ...
                                throw ireclib::error("Invalid gender string in box attribute.");
                        }
                    ...
                    else if (name == "part" && ts.back() == "box")
                    {
                        ...
                        else throw ireclib::error("<part> missing required attribute 'x'");

                        if (atts.is_in_list("y")) temp.y() = sa = atts["y"];
                        else throw ireclib::error("<part> missing required attribute 'y'");
                        ...
                        if (atts.is_in_list("name")) 
                        {
                            ...
                            {
                                throw ireclib::error("<part> with name '" + atts["name"] + "' is defined more than one time in a single box.");
                            }
                        }
                        else 
                        {
                            throw ireclib::error("<part> missing required attribute 'name'");
                        }
                    }
                    else if (name == "image")
                    {
                        ...
                        else throw ireclib::error("<image> missing required attribute 'file'");
                ...
                {
                    throw ireclib::error("Error on line " + cast_to_string(line_number) + ": " + e.what());
                }
            }
    ...
    // ------------------------------------------------------------------------------------
        ...
        )
        {
            ...
            if (!fin)
                throw ireclib::error("ERROR: unable to open " + filename + " for reading.");
    ...
    // ------------------------------------------------------------------------------------
        ...
        const std::string get_decoded_string()
        {
            ireclib::base64 base64_coder;
            ireclib::compress_stream::kernel_1ea compressor;
            std::ostringstream sout;
