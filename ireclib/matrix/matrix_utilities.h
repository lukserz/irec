...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        for (long r = 0; r < m.nr(); ++r)
        {
            ...
            {
                type temp = m(r,c);
                if (ireclib::impl::magnitude(temp) > ireclib::impl::magnitude(max_val))
                    max_val = temp;
                if (ireclib::impl::magnitude(temp) < ireclib::impl::magnitude(min_val))
                    min_val = temp;
            }
        }
    }
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        for (long r = 0; r < m.nr(); ++r)
        {
            ...
            {
                type temp = m(r,c);
                if (ireclib::impl::magnitude(temp) > ireclib::impl::magnitude(val))
                {
                    val = temp;
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(m.size() > 0 && is_vector(m) == true, 
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(m.size() > 0 && is_vector(m), 
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(is_vector(m) == true, 
...
// I introduced this struct because it avoids an inane compiler warning from gcc
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_vector(m1) && is_vector(m2) && m1.size() == m2.size() &&
                    m1.size() > 0, 
    ...
    { 
        IRECLIB_ASSERT(m1.size() == m2.size(), 
            "\t type dot(const matrix_exp& m1, const matrix_exp& m2)"
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(m.nr() > R && R >= 0 && m.nc() > C && C >= 0, 
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_lessthan, impl::lessthan, 1);
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_lessthan_eq, impl::lessthan_eq, 1);
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_greaterthan, impl::greaterthan, 1);
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_greaterthan_eq, impl::greaterthan_eq, 1);
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_equal_to, impl::equal_to, 1);
...
// ----------------------------------------------------------------------------------------
    ...
    IRECLIB_DEFINE_OP_MS(op_not_equal_to, impl::not_equal_to, 1);
...
// this is a workaround for a bug in visual studio 7.1
    ...
    )
    {
        ...
        IRECLIB_ASSERT(m.size() > 1 && is_col_vector(m), 
        ...
        {
            IRECLIB_ASSERT(m(0).size() == m(i).size() && m(i).size() > 0 && is_col_vector(m(i)), 
                   "\tconst matrix covariance(const matrix_exp& m)"
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(mat.nr() >= 0 && mat.nc() >= 0, 
...
// ----------------------------------------------------------------------------------------
    ...
    const matrix_op<op_add_diag<EXP> > operator+ (
        ...
        const matrix_exp<matrix_diag_op<op_identity_matrix_2<T> > >& IRECLIB_IF_ASSERT(rhs)
    )
    {
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        >
    const matrix_op<op_add_diag<EXP> > operator+ (
        const matrix_exp<matrix_diag_op<op_identity_matrix_2<T> > >& IRECLIB_IF_ASSERT(lhs), 
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        ...
        >
    const matrix_op<op_add_diag<EXP> > operator+ (
        const matrix_exp<matrix_diag_op<op_identity_matrix<T,N> > >& IRECLIB_IF_ASSERT(lhs), 
        const matrix_exp<EXP>& rhs
    )
    {
        // both matrices must contain the same type of element
    ...
    const matrix_op<op_add_diag<EXP> > operator+ (
        const matrix_exp<EXP>& lhs,
        const matrix_exp<matrix_diag_op<op_identity_matrix<T,N> > >& IRECLIB_IF_ASSERT(rhs)
    )
    {
        // both matrices must contain the same type of element
        COMPILE_TIME_ASSERT((is_same_type<T,typename EXP::type>::value == true));
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        COMPILE_TIME_ASSERT(EXP1::NR == EXP2::NR || EXP1::NR == 0 || EXP2::NR == 0);
        COMPILE_TIME_ASSERT(EXP1::NC == EXP2::NC || EXP1::NC == 0 || EXP2::NC == 0);
        IRECLIB_ASSERT(a.nr() == b.nr() && a.nc() == b.nc(),
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(m.size() == rows*cols && rows > 0 && cols > 0, 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_vector(v) == true && v.size() == m.nc(), 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_vector(v) == true && v.size() == m.nr(), 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_col_vector(v) == true && v.size() == m.nc(), 
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_vector(joints) && joints.size() >= 2, 
            "\t matrix_exp linpiece()"
            << "\n\t Invalid inputs were given to this function "
        ...
        {
            IRECLIB_ASSERT(joints(i-1) < joints(i), 
                "\t matrix_exp linpiece()"
                << "\n\t Invalid inputs were given to this function "
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(a.nr() == b.nr() || a.size() == 0 || b.size() == 0,
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(a.nc() == b.nc() || a.size() == 0 || b.size() == 0,
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT_HAS_STANDARD_LAYOUT(T);
