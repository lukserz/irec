// Copyright (C) 2009  Davis E. King (davis@ireclib.net)
...
// ----------------------------------------------------------------------------------------
    ...
    !*/
    {
        IRECLIB_ASSERT(A.nr() >= (long)l, "Invalid inputs were given to this function.");
...
// ----------------------------------------------------------------------------------------
    ...
    !*/
    {
        IRECLIB_ASSERT(A.size() >= l, "Invalid inputs were given to this function.");
        Q.set_size(A.size(), l);
...
// ----------------------------------------------------------------------------------------
    ...
    namespace simpl
    {
        ...
        )
        {
            ...
            IRECLIB_ASSERT(l > 0 && A.size() > 0 && n > 0, 
                "\t void svd_fast()"
                << "\n\t Invalid inputs were given to this function."
                << "\n\t l: " << l 
...
// ----------------------------------------------------------------------------------------
    ...
    struct det_helper<EXP,4>
    {
        ...
        )
        {
            ...
            type temp = m(0,0)*(ireclib::det(removerc<0,0>(m))) -
                        m(0,1)*(ireclib::det(removerc<0,1>(m))) +
                        m(0,2)*(ireclib::det(removerc<0,2>(m))) -
                        m(0,3)*(ireclib::det(removerc<0,3>(m)));
            return temp;
        }
    };
...
// ----------------------------------------------------------------------------------------
    ...
    template <
        typename EXP 
        >
    ireclib::vector<double,2> max_point_interpolated (
        const matrix_exp<EXP>& m
    )
    {
        IRECLIB_ASSERT(m.size() > 0, 
            "\tireclib::vector<double,2> point max_point_interpolated(const matrix_exp& m)"
            << "\n\tm can't be empty"
            << "\n\tm.size():   " << m.size() 
            << "\n\tm.nr():     " << m.nr() 
            << "\n\tm.nc():     " << m.nc() 
        ...
        if (m.nc()==1)
        {
            ...
            if (0 < pos && pos+1 < m.nr())
            {
                double v1 = ireclib::impl::magnitude(m(pos-1));
                double v2 = ireclib::impl::magnitude(m(pos));
                double v3 = ireclib::impl::magnitude(m(pos+1));
        ...
        for (long r = -1; r <= +1; ++r)
        {
            for (long c = -1; c <= +1; ++c)
            {
                pix(i) = ireclib::impl::magnitude(m(p.y()+r,p.y()+c));
                /*
                A(i,0) = c*c;
                A(i,1) = c*r;
                A(i,2) = r*r;
        ...
        g = w(3), 
            w(4);
        const ireclib::vector<double,2> delta = -inv(H)*g;

        // if delta isn't in an ascent direction then just use the normal max point.
        if (dot(delta, g) < 0)
            return p;
        else
            return vector<double,2>(p)+ireclib::clamp(delta, -1, 1);
    }
