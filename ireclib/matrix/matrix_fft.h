...
// ----------------------------------------------------------------------------------------
    ...
    matrix<std::complex<T>,NR,fftr_nc_size(NC),MM,L> fftr (const matrix<T,NR,NC,MM,L>& in)
    {
        //real FFT
        static_assert(std::is_floating_point<T>::value, "only support floating point types");
        IRECLIB_ASSERT(in.nc() % 2 == 0, "last dimension " << in.nc() << " needs to be even otherwise ifftr(fftr(data)) won't have matching dimensions");
...
// ----------------------------------------------------------------------------------------
    ...
    namespace details
    {
        struct fft_func
        {
            ...
            auto operator()(const MAT& mat) const { return ireclib::fft(mat); }

            template<typename MAT, typename T = typename MAT::type, typename std::enable_if<!is_complex<T>::value, bool>::type = true>
            auto operator()(const MAT& mat) const { return ireclib::fft(ireclib::complex_matrix(mat)); }
        ...
        struct fftr_func
        {
            ...
            auto operator()(const MAT& mat) const { return ireclib::fftr(mat); }

            static constexpr std::size_t freqsize(std::size_t fftsize) { return ireclib::fftr_nc_size(fftsize); }
        };

        struct ifft_func
        {
            template<typename MAT>
            auto operator()(const MAT& mat) const { return ireclib::ifft(mat); }
        };

        struct ifftr_func
        {
            template<typename MAT>
            auto operator()(const MAT& mat) const { return ireclib::ifftr(mat); }
        };
        ...
        )
        {
            ...
            IRECLIB_ASSERT(is_vector(signal), "input must be a vector type");
            IRECLIB_ASSERT(signal.size() >= (long)wlen, "signal.size() >= wlen not satisfied");
            IRECLIB_ASSERT(fftsize >= wlen, "fftsize >= wlen not satisfied");
            IRECLIB_ASSERT(wlen >= hoplen, "wlen >= hoplen not satisfied");
        ...
        )
        {
            ...
            IRECLIB_ASSERT(stft.nc() > 0 && stft.nr() > 0, "stft must be non-empty");
            IRECLIB_ASSERT(ifftr_nc_size(stft.nc()) >= (long)wlen, "fftsize >= wlen not satisfied");
            IRECLIB_ASSERT(wlen >= hoplen, "wlen >= hoplen not satisfied");
            ...
            IRECLIB_ASSERT(sum(subm(norm, 0, wlen/2, 1, ntime - wlen) < 1e-13) == 0, "NOLA constraint not satisfied");
...
// ----------------------------------------------------------------------------------------
    ...
    {
        return stft(ireclib::mat(signal), w, fftsize, wlen, hoplen);
    }
...
// ----------------------------------------------------------------------------------------
    ...
    {
        return stftr(ireclib::mat(signal), w, fftsize, wlen, hoplen);
    }
