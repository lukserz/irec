// Copyright (C) 2025 Davis E. King (davis@ireclib.net)
...
namespace ireclib
{
    ...
    public:
        ...
        {
            IRECLIB_CASSERT(vocab_size >= BPE_TOKENIZER_BASE_VOCAB_SIZE);
        ...
        friend void deserialize(bpe_tokenizer& tok, std::istream& in) {
            std::string version;
            ireclib::deserialize(version, in);
            if (version != "bpe_tokenizer2_")
                throw ireclib::serialization_error("Unexpected version '" + version + "' found while deserializing ireclib::bpe_tokenizer_.");
