...
// ----------------------------------------------------------------------------------------
    ...
    void deserialize(frobmetric_training_sample<matrix_type>& item, std::istream& in)
    {
        ...
        if (version != 1)
            throw serialization_error("Unexpected version found while deserializing ireclib::frobmetric_training_sample.");
    ...
    public:
        ...
        )
        {
                ...
                );
            {
                ...
                IRECLIB_ASSERT(dims != 0,
                    "\tvoid vector_normalizer_frobmetric::train()"
                    << "\n\t The dimension of the input vectors can't be zero."
                    );
                for (unsigned long i = 0; i < samples.size(); ++i)
                {
                    IRECLIB_ASSERT(is_col_vector(samples[i].anchor_vect), 
                        "\tvoid vector_normalizer_frobmetric::train()"
                        << "\n\t Invalid inputs were given to this function."
                        << "\n\t i: " << i
                        );
                    IRECLIB_ASSERT(samples[i].anchor_vect.size() == dims, 
                        "\tvoid vector_normalizer_frobmetric::train()"
                        << "\n\t Invalid inputs were given to this function."
                        << "\n\t i:    " << i
                        << "\n\t dims: " << dims
                    ...
                    IRECLIB_ASSERT(samples[i].num_triples() != 0,
                        "\tvoid vector_normalizer_frobmetric::train()"
                    ...
                    for (unsigned long j = 0; j < samples[i].near_vects.size(); ++j)
                    {
                        IRECLIB_ASSERT(is_col_vector(samples[i].near_vects[j]), 
                            "\tvoid vector_normalizer_frobmetric::train()"
                            << "\n\t Invalid inputs were given to this function."
                            << "\n\t i: " << i
                            << "\n\t j: " << j
                            );
                        IRECLIB_ASSERT(samples[i].near_vects[j].size() == dims, 
                            "\tvoid vector_normalizer_frobmetric::train()"
                            << "\n\t Invalid inputs were given to this function."
                            << "\n\t i:    " << i
                            << "\n\t j:    " << j
                    ...
                    {
                        IRECLIB_ASSERT(is_col_vector(samples[i].far_vects[j]), 
                            "\tvoid vector_normalizer_frobmetric::train()"
                            << "\n\t Invalid inputs were given to this function."
                            << "\n\t i: " << i
                            << "\n\t j: " << j
                            );
                        IRECLIB_ASSERT(samples[i].far_vects[j].size() == dims, 
                            "\tvoid vector_normalizer_frobmetric::train()"
                            << "\n\t Invalid inputs were given to this function."
                            << "\n\t i:    " << i
                            << "\n\t j:    " << j
            ...
            IRECLIB_ASSERT(is_finite(m), "Some of the input vectors to vector_normalizer_frobmetric::train() have infinite or NaN values");
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(in_vector_size() != 0 && in_vector_size() == x.size() && 
...
// ----------------------------------------------------------------------------------------
    ...
    )   
    {
        ...
        if (version != 1 && version != 2)
            throw serialization_error("Unsupported version found while deserializing ireclib::vector_normalizer_frobmetric.");
