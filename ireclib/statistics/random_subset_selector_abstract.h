...
namespace ireclib
{
    template <
        typename T,
        typename Rand_type = ireclib::rand
        >
    class random_subset_selector
    {
        /*!
            ...
            REQUIREMENTS ON Rand_type
                must be an implementation of ireclib/rand/rand_kernel_abstract.h
...
// ----------------------------------------------------------------------------------------
    ...
    /*!
        requires
            - random_seed must be convertible to a string by ireclib::cast_to_string()
        ensures
            - returns a random subset R such that:
                - R contains a random subset of the given samples
                - R.size() == min(num, samples.size())
