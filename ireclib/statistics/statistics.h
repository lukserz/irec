// Copyright (C) 2008  Davis E. King (davis@ireclib.net), Steve Taylor
...
// ----------------------------------------------------------------------------------------
    ...
    ) 
    {
        ...
        if (version != 2)
            throw ireclib::serialization_error("Unexpected version number found while deserializing ireclib::running_stats object.");
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            IRECLIB_ASSERT(decay_halflife > 0);
...
// ----------------------------------------------------------------------------------------
    ...
    ) 
    {
        ...
        deserialize(version, in);
        if (version != 1)
            throw ireclib::serialization_error("Unexpected version number found while deserializing ireclib::running_stats_decayed object.");
...
// ----------------------------------------------------------------------------------------
    ...
    {
        // make sure requires clause is not broken
        IRECLIB_ASSERT(a.size() == b.size() && a.size() > 1,
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(((long)max_index_plus_one(val) <= in_vector_size() && in_vector_size() > 0),
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(is_col_vector(val) && (in_vector_size() == 0 || val.size() == in_vector_size()),
                "\t void running_covariance::add()"
                << "\n\t Invalid inputs were given to this function"
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( in_vector_size() != 0,
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( in_vector_size() != 0 && current_n() > 1,
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT((in_vector_size() == 0 || item.in_vector_size() == 0 || in_vector_size() == item.in_vector_size()),
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( x_size > 0 && y_size > 0,
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( ((long)max_index_plus_one(x) <= x_vector_size() && x_vector_size() > 0) &&
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT( (is_col_vector(x) && x.size() == x_vector_size() && x_vector_size() > 0) &&
                         ((long)max_index_plus_one(y) <= y_vector_size() && y_vector_size() > 0) ,
                "\t void running_cross_covariance::add()"
                << "\n\t Invalid inputs were given to this function"
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(is_col_vector(x) && (x_vector_size() == 0 || x.size() == x_vector_size()) &&
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT((x_vector_size() == 0 || item.x_vector_size() == 0 || x_vector_size() == item.x_vector_size()) &&
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        )
        {
            ...
            IRECLIB_ASSERT(is_finite(m), "Some of the input vectors to vector_normalizer::train() have infinite or NaN values");
        }
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(x.nr() == in_vector_size() && x.nc() == 1,
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        )
        {
            ...
            IRECLIB_ASSERT(is_finite(m), "Some of the input vectors to vector_normalizer_pca::train() have infinite or NaN values");
        }
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(k1 <= n1, "k1: "<< k1 << "  n1: "<< n1);
        IRECLIB_ASSERT(k2 <= n2, "k2: "<< k2 << "  n2: "<< n2);
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_ASSERT(AB_count <= A_count && A_count <= total_num_observations,
            "AB_count: " << AB_count << ", A_count: "<< A_count << ", total_num_observations: " << total_num_observations);
        IRECLIB_ASSERT(AB_count <= B_count && B_count <= total_num_observations,
            "AB_count: " << AB_count << ", B_count: "<< B_count << ", total_num_observations: " << total_num_observations);

        if (total_num_observations == 0)
            return 0;

        IRECLIB_ASSERT(A_count + B_count - AB_count <= total_num_observations,
