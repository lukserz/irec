...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_CASSERT(class_labels.size() > 1,
            "\t void compute_lda_transform()"
            << "\n\t You can't call this function if the number of distinct class labels is less than 2."
            );
        IRECLIB_CASSERT(X.size() != 0 && (long)row_labels.size() == X.nr() && lda_dims != 0,
            "\t void compute_lda_transform()"
...
// ----------------------------------------------------------------------------------------
    ...
    {
        IRECLIB_CASSERT(true_detections.size() != 0);
        IRECLIB_CASSERT(false_detections.size() != 0);
