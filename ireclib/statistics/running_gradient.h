...
namespace ireclib
{
    ...
    public:
        ...
        friend void deserialize (running_gradient& item, std::istream& in)
        {
            ...
            if (version != 1)
                throw serialization_error("Unexpected version found while deserializing ireclib::running_gradient.");
...
// ----------------------------------------------------------------------------------------
    ...
    {
        // make sure requires clause is not broken
        IRECLIB_ASSERT(0.5 < probability_of_decrease && probability_of_decrease < 1,
            "\t size_t count_steps_without_decrease()"
...
// ----------------------------------------------------------------------------------------
    ...
    {
        // make sure requires clause is not broken
        IRECLIB_ASSERT(0 <= quantile_discard && quantile_discard <= 1);
        IRECLIB_ASSERT(0.5 < probability_of_decrease && probability_of_decrease < 1,
            "\t size_t count_steps_without_decrease_robust()"
            << "\n\t probability_of_decrease: "<< probability_of_decrease 
        );
...
// ----------------------------------------------------------------------------------------
    ...
    {
        // make sure requires clause is not broken
        IRECLIB_ASSERT(0.5 < probability_of_increase && probability_of_increase < 1,
            "\t size_t count_steps_without_increase()"
