...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(is_col_vector(x) && is_col_vector(y) && 
                         x.size() == y.size() &&
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(is_col_vector(x) && (in_vector_size() == 0 || x.size() == in_vector_size()),
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(0 < num_rows && num_rows <= in_vector_size(),
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(0 < eps && eps <= 1 && in_vector_size() != 0,
