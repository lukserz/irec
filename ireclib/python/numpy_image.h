    ...
    )
    {
        ...
        if (expected_channels == 1)
        {
            if (!(img.ndim() == 2 || (img.ndim()==3&&img.shape(2)==1)))
                throw ireclib::error("Expected a 2D numpy array, but instead got one with " + std::to_string(img.ndim()) + " dimensions.");
        }
        else
        {
            if (img.ndim() != 3)
            {
                throw ireclib::error("Expected a numpy array with 3 dimensions, but instead got one with " + std::to_string(img.ndim()) + " dimensions.");
            }
            else if (img.shape(2) != expected_channels)
            {
                if (pixel_traits<pixel_type>::rgb)
                    throw ireclib::error("Expected a RGB image with " + std::to_string(expected_channels) + " channels but got an image with " + std::to_string(img.shape(2)) + " channels.");
                else
                    throw ireclib::error("Expected an image with " + std::to_string(expected_channels) + " channels but got an image with " + std::to_string(img.shape(2)) + " channels.");
            }
        }
    }
    ...
    )
    {
        if (!is_image<pixel_type>(obj))
        {
            ...
            auto toname = [](char type, size_t size) {
                ...
                else IRECLIB_CASSERT(false, "unknown type");
            };

            throw ireclib::error("Expected numpy array with elements of type " + std::string(toname(expected_type,expected_size)) + " but got " + toname(got_type, got_size) + ".");
        }
    }
    ...
    class numpy_image : public py::array_t<typename pixel_traits<pixel_type>::basic_pixel_type, py::array::c_style>
    {
        /*!
            REQUIREMENTS ON pixel_type
                - is a ireclib pixel type, this just means that ireclib::pixel_traits<pixel_type>
                  is defined.

            WHAT THIS OBJECT REPRESENTS
                This is an image object that implements ireclib's generic image interface and
                ...
                array_t object with the additional routines needed to conform to ireclib's
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        else IRECLIB_CASSERT(false, "Unsupported pixel type used in assign_image().");
    }
...
// ----------------------------------------------------------------------------------------
    ...
    size_t width_step (const numpy_image<pixel_type>& img)
    {
        ...
        if (img.ndim()==3 && img.strides(2) != sizeof(basic_pixel_type))
            throw ireclib::error("The stride of the 3rd dimension (the channel dimension) of the numpy array must be " + std::to_string(sizeof(basic_pixel_type)));
        if (img.strides(1) != sizeof(pixel_type))
            throw ireclib::error("The stride of the 2nd dimension (the columns dimension) of the numpy array must be " + std::to_string(sizeof(pixel_type)));
...
namespace pybind11
{
    namespace detail
    {
        template <typename pixel_type> struct handle_type_name<ireclib::numpy_image<pixel_type>> 
        {
            using basic_pixel_type = typename ireclib::pixel_traits<pixel_type>::basic_pixel_type;

            static constexpr auto getname()
            {
                constexpr long channels = ireclib::pixel_traits<pixel_type>::num;
        ...
        template <typename pixel_type>
        struct pyobject_caster<ireclib::numpy_image<pixel_type>> {
            using type = ireclib::numpy_image<pixel_type>;
            ...
            bool load(handle src, bool /*convert*/) {
                ...
                if (!ireclib::is_image<pixel_type>(temp))
                    return false;
                value = temp;
