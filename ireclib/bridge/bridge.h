...
// ---------------------------------------------------------------------------------------- 
    ...
    struct connect_to_ip_and_port
    {
        ...
        ): ip(ip_), port(port_)
        {
            ...
            IRECLIB_ASSERT(is_ip_address(ip) && port != 0,
                "\t connect_to_ip_and_port()"
        ...
        private:
            ...
            )
            {
                while (true)
                {
                    ...
                    {
                        dlog << LERROR << "ireclib::serialization_error thrown while deserializing message from " 
                            << con->get_foreign_ip() << ":" << con->get_foreign_port() 
            ...
            )
            {
                while (true)
                {
                    ...
                    {
                        dlog << LERROR << "ireclib::serialization_error thrown while serializing message to " 
                            << con->get_foreign_ip() << ":" << con->get_foreign_port() 
                            << ".\nThe exception error message is: \n" << e.what();
                    }
                    catch (std::exception& e)
