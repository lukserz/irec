...
namespace ireclib
{
    ...
    class rand
    {
        ...
        public:
            ...
            // These typedefs are here for backwards compatibility with older versions of ireclib.
            ...
            )
            {
                ...
                const double rndmax = std::numeric_limits<ireclib::uint32>::max();
            ...
            {
                IRECLIB_ASSERT(lambda > 0, "lambda must be greater than zero");
                double u = 0.0;
                while (u == 0.0)
                    u = get_random_double();
            ...
            {
                IRECLIB_ASSERT(k > 0, "k must be greater than zero");
                IRECLIB_ASSERT(lambda > 0, "lambda must be greater than zero");
                double u = 0.0;
                while (u == 0.0)
                    u = get_random_double();
            ...
            {
                IRECLIB_CASSERT(alpha > 0, "alpha must be greater than zero")
                IRECLIB_CASSERT(beta > 0, "beta must be greater than zero");
