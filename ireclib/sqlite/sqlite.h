...
// --------------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(db_.is_open() == true,
                        "\t statement::statement()"
                        << "\n\t The database must be opened before calling this routine."
                        << "\n\t this: " << this
            );
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(idx < get_num_columns(),
        ...
        {
            // make sure requires clause is not broken
            IRECLIB_ASSERT(1 <= parameter_id && parameter_id <= get_max_parameter_id(),
