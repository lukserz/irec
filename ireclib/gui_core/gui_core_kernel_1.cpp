// Copyright (C) 2005  Davis E. King (davis@ireclib.net), Keita Mochizuki
...
// ----------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------
        ...
        const std::shared_ptr<ireclib::mutex>& global_mutex()
        {
            static std::shared_ptr<ireclib::mutex> m(new ireclib::mutex);
            return m;
        }
        ...
        public:
            ...
            std::shared_ptr<ireclib::mutex> reference_to_global_mutex;

            event_handler_thread(
            ) :
                dlog("ireclib.gui_core"),
                hInstance(0),
                helper_window(0),
        ...
        private:
            ...
            )
            {
                ...
                wndclass.lpfnWndProc   = ireclib::gui_core_kernel_1_globals::WndProc ;
        ...
        // Note that we need to use ireclib::shared_ptr_thread_safe here rather than
        ...
        // finishes.  
        // 
        // ireclib::shared_ptr_thread_safe doesn't have this problem.  An alternative would be
        ...
        // windows.  So in any case, we just use the old ireclib::shared_ptr_thread_safe to
        // avoid this problem.
        ...
        )
        {
            try
            {
                if (nonblocking)
                {
                    ...
                    param->text = text;
                    param->title = title;
                    ireclib::create_new_thread(error_box_helper,param);
                }
                else
                {
                    ...
                    case WM_CLOSE:
                        {
                            ...
                            {
                                IRECLIB_ASSERT(win->has_been_destroyed == false,
...
// ----------------------------------------------------------------------------------------
    ...
    {
        using namespace gui_core_kernel_1_globals;
        IRECLIB_ASSERT(!(undecorated == true && resizable == true),
