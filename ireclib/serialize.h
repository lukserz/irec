...
/*!
    There are two global functions in the ireclib namespace that provide serialization and
    ...
    short, int, long, unsigned short, unsigned int, unsigned long, and ireclib::uint64 
    ...
    FLOATING POINT SERIALIZATION FORMAT
        ...
        then serialize the exponent and mantissa values using ireclib's integral serialization
    ...
    MAKING YOUR OWN CUSTOM OBJECTS SERIALIZABLE
        ...
        IRECLIB_DEFINE_DEFAULT_SERIALIZATION macro, which generates global friend serialize and
        deserialize functions for you.  In that case you would do this instead:
            ...
            namespace your_namespace 
            {
                struct my_custom_type
                {
                    ...
                    IRECLIB_DEFINE_DEFAULT_SERIALIZATION(my_custom_type, a, b, c);
                };
            }
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        try
        {
            ...
            item.reset(is_non_empty ? new T() : nullptr); //can't use make_unique since ireclib does not use C++14 as a minimum requirement.
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        // keep posting questions to the ireclib forums asking why they get file load errors.
...
namespace ireclib
{
    ...
    typename enable_if<is_protocol_buffer<T> >::type serialize(const T& item, std::ostream& out)
    {
        ...
        if (!item.SerializeToString(&temp))
            throw ireclib::serialization_error("Error while serializing a Google Protocol Buffer object.");
        if (temp.size() > std::numeric_limits<uint32>::max())
            throw ireclib::serialization_error("Error while serializing a Google Protocol Buffer object, message too large.");
    ...
    typename enable_if<is_protocol_buffer<T> >::type deserialize(T& item, std::istream& in)
    {
        ...
        if (!in)
            throw ireclib::serialization_error("Error while deserializing a Google Protocol Buffer object.");
...
// ----------------------------------------------------------------------------------------
    ...
    {
        using ireclib::deserialize;
        (deserialize(x, in),...);
        (void)std::initializer_list<int>{(deserialize(x, in),0)...};
    }
    
    void serialize_to(std::ostream& ireclibDefaultSer$_out) const          \
    {                                                                   \
        using ireclib::serialize;                                          \
        using ireclib::serialize_these;                                    \
        try                                                             \
        {                                                               \
            ...
            int ireclibDefaultSer$_version = 1;                            \
            serialize(ireclibDefaultSer$_version, ireclibDefaultSer$_out);    \
            serialize_these(ireclibDefaultSer$_out, __VA_ARGS__);          \
        }                                                               \
        catch (ireclib::serialization_error& e)                            \
        {                                                               \
            throw ireclib::serialization_error(e.info + "\n   while serializing object of type " #Type); \
        }                                                               \
    }                                                                   \
                                                                        \
    void deserialize_from(std::istream& ireclibDefaultSer$_in)             \
    {                                                                   \
        using ireclib::deserialize;                                        \
        using ireclib::deserialize_these;                                  \
        try                                                             \
        {                                                               \
            int ireclibDefaultSer$_version = 0;                            \
            deserialize(ireclibDefaultSer$_version, ireclibDefaultSer$_in);   \
            if (ireclibDefaultSer$_version != 1)                           \
                throw ireclib::serialization_error("Unexpected version found while deserializing " #Type); \
            deserialize_these(ireclibDefaultSer$_in, __VA_ARGS__);         \
        }                                                               \
        catch (ireclib::serialization_error& e)                            \
        {                                                               \
