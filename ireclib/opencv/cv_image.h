...
namespace ireclib
{
    ...
    public:
        ...
        {
            IRECLIB_CASSERT(img.depth() == cv::DataType<typename pixel_traits<pixel_type>::basic_pixel_type>::depth &&
...
// version.
        ...
        {
          IRECLIB_ASSERT(0<= column && column < nc(),
    ...
    private:
        ...
        {
            IRECLIB_CASSERT( img->dataOrder == 0, "Only interleaved color channels are supported with cv_image"); 
            IRECLIB_CASSERT((img->depth&0xFF)/8*img->nChannels == sizeof(pixel_type), 
