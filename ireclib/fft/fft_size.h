...
namespace ireclib
{
    ...
    public:
        ...
        !*/
        {
            ...
            IRECLIB_ASSERT(ndims > 0, "fft_size objects must be non-empty");
            IRECLIB_ASSERT(ndims <= _dims.size(), "fft_size objects must have size less than 6");
            IRECLIB_ASSERT(std::find_if(dims_begin, dims_end, [](long dim) {return dim <= 0;}) == dims_end, "fft_size objects must contain strictly positive values");
        ...
        !*/
        {
            IRECLIB_ASSERT(index < _size, "index " << index << " out of range [0," << _size << ")");
            return _dims[index];
        }
        ...
        !*/
        {
            IRECLIB_ASSERT(_size > 0, "object is empty");
    ...
    inline ireclib::uint32 hash(
        const fft_size& item,
        ireclib::uint32 seed = 0)
    {
        seed = ireclib::hash((ireclib::uint64)item.num_dims(), seed);
        seed = std::accumulate(item.begin(), item.end(), seed, [](ireclib::uint32 seed, long next) {
            return ireclib::hash((ireclib::uint64)next, seed);
        });
        return seed;
    }
    /*!
    ...
    {
        IRECLIB_ASSERT(size.num_dims() > 0);
