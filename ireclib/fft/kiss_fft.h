...
{    
    namespace kiss_details
    {
        ...
        void kiss_fft_stride(const kiss_fft_state<T>& cfg, const std::complex<T>* in, std::complex<T>* out,int fin_stride)
        {
            if (in == out) 
            {
                IRECLIB_ASSERT(out != nullptr, "out buffer is NULL!");
        ...
        {
            IRECLIB_ASSERT((key.dims[0] & 1) == 0, "real FFT must have even dimension");
        ...
        {
            IRECLIB_ASSERT(!plan.substate.inverse, "bad fftr plan : need a forward plan. This is an inverse plan");
        ...
        {
            IRECLIB_ASSERT(plan.substate.inverse, "bad Ifftr plan : need an inverse plan. This is a forward plan")
