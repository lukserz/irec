...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        IRECLIB_ASSERT(is_ordered_by_index(edges),
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        const unsigned long num_nodes = max_index_plus_one(edges);
        // make sure requires clause is not broken
        IRECLIB_ASSERT(labels.size() == num_nodes,
                    "\t double modularity()"
                    << "\n\t Invalid inputs were given to this function"
        );

        unsigned long num_labels;
        const std::vector<unsigned long>& labels_ = ireclib::impl::remap_labels(labels,num_labels);
