...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        while (line != "\r")
        {
            ...
            {
                first_part_of_header = ireclib::trim(line.substr(0, position_of_double_point));

                if ( !incoming_headers[first_part_of_header].empty() )
                    incoming_headers[ first_part_of_header ] += " ";
                incoming_headers[first_part_of_header] += ireclib::trim(line.substr(position_of_double_point+1));
...
// ----------------------------------------------------------------------------------------
    ...
    const logger server_http::dlog("ireclib.server_http");
