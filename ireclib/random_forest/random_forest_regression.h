// Copyright (C) 2018  Davis E. King (davis@ireclib.net)
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            IRECLIB_CASSERT(x.size() > 0);
            IRECLIB_CASSERT(x.size() == y.size());
            for (auto& el : x)
                IRECLIB_CASSERT(el.size() == x[0].size(), "All the vectors in a training set have to have the same dimensionality.");

            IRECLIB_CASSERT(x[0].size() != 0, "The vectors can't be empty.");
        ...
        {
            IRECLIB_ASSERT(max_num_feats() != 0);
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        {
            IRECLIB_ASSERT(trees.size() > 0);
            IRECLIB_ASSERT(trees.size() == leaves.size(), "Every set of tree nodes has to have leaves");
            for (size_t i = 0; i < trees.size(); ++i)
            {
                IRECLIB_ASSERT(trees[i].size() > 0, "A tree can't have 0 leaves.");
                for (auto& node : trees[i])
                {
                    IRECLIB_ASSERT(trees[i].size()+leaves[i].size() > node.left, "left node index in tree is too big. There is no associated tree node or leaf.");
                    IRECLIB_ASSERT(trees[i].size()+leaves[i].size() > node.right, "right node index in tree is too big. There is no associated tree node or leaf.");
                }
            }
        }
        ...
        {
            IRECLIB_ASSERT(get_num_trees() > 0);
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        ) const 
        {
            ...
            IRECLIB_CASSERT(fe.max_num_feats() != 0);
            ...
            {
                ireclib::rand rnd(random_seed + std::to_string(i));
                ...
                while(ranges_to_process.size() > 0)
                {
                    ...
                    IRECLIB_ASSERT(left_split.begin < left_split.end);
                    IRECLIB_ASSERT(right_split.begin < right_split.end);
                ...
                // tree.size() value.
                IRECLIB_CASSERT(max_num_nodes >= tree.size()); 
