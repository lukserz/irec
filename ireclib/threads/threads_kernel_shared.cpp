...
// ----------------------------------------------------------------------------------------
            ...
            do_not_ever_destruct(false)
        {
            ...
            // similar weird issues.  So when we are using ireclib on windows we just disable
    ...
    // ------------------------------------------------------------------------------------
        ...
        )
        {
            ...
            while (self.destruct == false)
            {
                ...
                if (self.data_ready.wait_or_timeout(IRECLIB_THREAD_POOL_TIMEOUT) == false && 
                    self.function_pointer == 0)
                    break;
...
// ----------------------------------------------------------------------------------------
    ...
    {
        return threads_kernel_shared::thread_pool().is_ireclib_thread(id);
    }

    bool is_ireclib_thread (
    )
    {
        return is_ireclib_thread(get_thread_id());
    }
