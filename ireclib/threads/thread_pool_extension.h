...
// ----------------------------------------------------------------------------------------
    ...
    inline void swap (
        future<T>& a,
        future<T>& b
    ) { ireclib::exchange(a.get(), b.get()); }
    // Note that ireclib::exchange() just calls std::swap.  I'm only using it because
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        )
        {
            ...
            {
                std::cerr << "An unhandled exception was inside a ireclib::thread_pool when it was destructed." << std::endl;
